...
User Function LOJRNFCe(	oNFCe		, oProt		, nDecimais	, aFormas	,;
	...
	*/
	...
	if Alltrim(FunName()) $ 'MV05002|MATA410|FINA740|FATWS001|RD06010'
	//IF Valtype(lGerPDF) = 'L'
	...
	_cXNomArq:="nfc-e_"+cChvNFCe
	ferase('C:\TEMP\'+Alltrim(_cXNomArq)+'.pdf')
	oPrint:= FWMSPrinter():New(_cXNomArq, IMP_PDF, lAdjustToLegacy,, lDisableSetup,,,,,,,.T.)
...
User Function LjRImpNFCE(	oNFCe		, oProt		, nDecimais	, aFormas	,; 
	...
	Local oFont, oFont1, oFont2, oFont3, oFont4, oFont5, oFont09, oFont09N, oFont10, oFont10N, oFont12, oFont12N
	...
	//FONTE CUSTOMIZADOS
	oFont09		:= TFont():New("Courier New",,09,,.F. /*NEGRITO*/,,,,.T.,)	
	oFont09N	:= TFont():New("Courier New",,09,,.T. /*NEGRITO*/,,,,.T.,)	
	oFont10		:= TFont():New("Courier New",,09,,.F. /*NEGRITO*/,,,,.T.,)	
	oFont10N	:= TFont():New("Courier New",,09,,.T. /*NEGRITO*/,,,,.T.,)	
	oFont12		:= TFont():New("Courier New",,12,,.F. /*NEGRITO*/,,,,.T.,)	
	oFont12N	:= TFont():New("Courier New",,12,,.T. /*NEGRITO*/,,,,.T.,)	
	oFont14		:= TFont():New("Courier New",,14,,.F. /*NEGRITO*/,,,,.T.,)	
	oFont14N	:= TFont():New("Courier New",,14,,.T. /*NEGRITO*/,,,,.T.,)	
	oFont16		:= TFont():New("Courier New",,16,,.F. /*NEGRITO*/,,,,.T.,)	
	oFont16N	:= TFont():New("Courier New",,16,,.T. /*NEGRITO*/,,,,.T.,)	
	...
	*/
	...
	//LogoTipo
	oPrint:SayBitmap( nAuxLn, 0050, cBmp, 200, 200)
	
	//montando a caixa do comprovante de entrega de cupom
	oPrint:Box(nAuxLn,0080,nAuxLn+26,2320)
	oPrint:Box(nAuxLn,1970,nAuxLn+130,2320)
	oPrint:Box(nAuxLn+30,0080,nAuxLn+130,0450)
	oPrint:Box(nAuxLn+30,0450,nAuxLn+130,1970)
/*	oPrint:Box(0025,0080,0050,2320)
	oPrint:Box(0025,1970,0155,2320)
	oPrint:Box(0056,0080,0157,0450)
	oPrint:Box(0056,0450,0157,1970)
*/
	cTextoAux := "RECEBEMOS DE "+AllTrim(aEmitNfce:_XNOME:TEXT)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO"
	nAuxLn+=20
	oPrint:Say(nAuxLn,0085, cTextoAux, oFont09 )
	//oPrint:Say(0045,0085, cTextoAux, oFont09 )


	cTextoAux := 'NFC-e'
	nAuxLn+=10
	oPrint:Say(nAuxLn,2085, cTextoAux, oFont12N )
	//oPrint:Say(0055,2085, cTextoAux, oFont12N )

	cTextoAux := 'DATA DE RECEBIMENTO'
	nAuxLn+=20
	oPrint:Say(nAuxLn,0085, cTextoAux, oFont09 )
//	oPrint:Say(0075,0085, cTextoAux, oFont09 )

	cTextoAux := 'IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR'
	oPrint:Say(nAuxLn,0455, cTextoAux, oFont09 )
//	oPrint:Say(0075,0455, cTextoAux, oFont09 )

	cTextoAux := 'Nº...: '+StrZero(Val(aIdNfce:_NNF:TEXT),9)
	nAuxLn+=15
	oPrint:Say(nAuxLn,1990, cTextoAux, oFont12 )
//	oPrint:Say(0090,1990, cTextoAux, oFont12 )

	cTextoAux := 'SÉRIE: '+StrZero(Val(aIdNfce:_SERIE:TEXT),3)
	nAuxLn+=35
	oPrint:Say(nAuxLn,1990, cTextoAux, oFont12 )
//	oPrint:Say(0125,1990, cTextoAux, oFont12 )
...
//	nAuxLn += 775
	...
	//Impressão do QrCode
	oPrint:QRCode( nAuxLn+500, 1700, cKeyQRCode, 100)
	
	cTextoAux	:= alltrim(aEmitNfce:_XNOME:TEXT)
	...
	oPrint:Say( nAuxLn, 0085, cTextoAux, oFont12 )
	nAuxLn += 40
	...
	oPrint:Say( nAuxLn, 0085, "DOCUMENTO AUXILIAR DA NOTA FISCAL DE CONSUMIDOR ELETRÔNICA", oFont12 )
//	oPrint:Say( nAuxLn, 0085, padc("DOCUMENTO AUXILIAR DA NOTA FISCAL DE CONSUMIDOR ELETRÔNICA",085), oFont14N )
	
	/* todo aumentar a fonte*/ 
	If lContigen
		nAuxLn += 60
		oPrint:Say( nAuxLn,0085, PadC("EMITIDA EM CONTINGÊNCIA", 66), oFont12N )
		nAuxLn += 40
		oPrint:Say( nAuxLn,0085, PadC("Pendente de autorização", 85), oFont12 )
	EndIf

	nAuxLn += 40	//quebra de linha entre as divisoes
	nAuxLn += 40	//quebra de linha entre as divisoes
	...
	Else
		...
		*/
		...
		EndIf		
		oPrint:Say( nAuxLn, aColDet[01, 02], cTextoAux, oFont12N )
		...
		If LjRTemNode(aDestNfce,"_ENDERDEST")
			...
			oPrint:Say( nAuxLn, aColDet[01, 02], cTextoAux, oFont12 )
		EndIf		
	EndIf	
	...
	If Type("oNfce:_NFE:_INFNFE:_INFADIC") == "O" .AND. LjRTemNode(oNfce:_NFE:_INFNFE:_INFADIC, "_INFCPL")
		...
		For nY := 1 to nInfCpl
			nAuxLn += 40
			oPrint:Say( nAuxLn, 0085, aInfCpl[nY], oFont12 )
	...
	*/
	...
	For nI := 1 to Len(aColDet)
		...
		oPrint:Say( nAuxLn,aColDet[nI, 02], upper(cColuna), oFont12N )
	Next nI
	...
	For nX := 1 to Len(aItemNfce)
		...
		oPrint:Say( nAuxLn,aColDet[01, 02],PADR(aItemNfce[nX]:_PROD:_CPROD:TEXT,aColDet[01, 03]) + " "  ,oFont12 )	//Codigo de Produto
		...
		oPrint:Say( nAuxLn,aColDet[02, 02],PADR(cDescrPro,nTamCol),oFont12 )					//Descricao de Produto
		oPrint:Say( nAuxLn,aColDet[03, 02],PADL(StrTran( AllTrim(Str(nItemQtde)),".", ","),aColDet[03, 03]) + space(1)	,oFont12 )	//Qtde
		oPrint:Say( nAuxLn,aColDet[04, 02],PADR(aItemNfce[nX]:_PROD:_UCOM:TEXT,aColDet[04, 03])	+ space(1)   ,oFont12 )	//Unidade de Medida
		oPrint:Say( nAuxLn,aColDet[05, 02],PadL(AllTrim(Transform(nItemUnit , aColDet[05, 04])),aColDet[05, 03]) + space(1)  	,oFont12 )	//Valor Unit.
		oPrint:Say( nAuxLn,aColDet[06, 02],PadL(AllTrim(Transform(nItemTotal, aColDet[06, 04])),aColDet[06, 03])			,oFont12 )	//Valor Total
		...
		If nValDeson > 0
			nAuxLn+=40
			oPrint:Say( nAuxLn,aColDet[02, 02],cMsgDeson, oFont12 )		//Mensagem de Desoneração
			oPrint:Say( nAuxLn,aColDet[06, 02],PadL(AllTrim(Transform(nValDeson, aColDet[06, 04])),aColDet[06, 03])			,oFont12 )	//Valor Desoneração
		EndIf
	...
	*/
	oPrint:SetFont(oFont12N) //Times New Roman - 13 - Negrito
	...
	*/
	oPrint:Say( nAuxLn, 1, PadC("Consulte pela chave de acesso em: ", 85), oFont12 )
	nAuxLn += 40
	
	//Link de consulta publica
	cURLNFCE := LjNFCeURL(cAmbiente,.T.)
	oPrint:Say( nAuxLn, 1, PadC(cURLNFCE,85), oFont12 )	  	
	nAuxLn += 40
	...
	Next	
	oPrint:Say( nAuxLn, 1, PadC(cTextoAux,85), oFont12 )
...
*/
	...
	*/	
	If Type("oNfce:_NFE:_INFNFE:_INFADIC") == "O" .AND. LjRTemNode(oNfce:_NFE:_INFNFE:_INFADIC, "_INFADFISCO")
		...
		For nY := 1 to nInfCpl
			...
			If nY <> 1			
				nAuxLn += 40
			EndIf
			oPrint:Say( nAuxLn, 0080, aInfCpl[nY], oFont12 )
		Next
		nAuxLn += 60
	EndIf
	...
	If cAmbiente == "2"		
		oPrint:Say( nAuxLn, 1, PadC("EMITIDA EM AMBIENTE DE HOMOLOGAÇÃO – SEM VALOR FISCAL",85), oFont12 )
		nAuxLn += 60
	EndIf

	If lContigen
		oPrint:Say( nAuxLn, 1, PadC("EMITIDA EM CONTINGÊNCIA",66), oFont12 )
		nAuxLn += 40
		oPrint:Say( nAuxLn, 1, PadC("Pendente de autorização",85), oFont12 )
		nAuxLn += 60
	EndIf
