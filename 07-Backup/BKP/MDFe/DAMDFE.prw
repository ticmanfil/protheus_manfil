...
Local aCab	:= {}
	...
	oDamdfe:StartPage()
		...
		//CUSTOMIZADO MANFIL
		DamdfeVCP( @oDamdfe, @nLin, aCab[nCont] ) 			  //Veiculo - Condutor - Pedagio
//		DamdfeVCP( @oDamdfe, @nLin ) 			  //Veiculo - Condutor - Pedagio
		//FIM CUSTOMIZADO MANFIL
...
Local nLinQrChav	:= 0
	...
	oDamdfe:Say(nLin-nPosIniCtg, 0075, "Número", oFont13)	//Numero
	...
	oDamdfe:Say(nLin-nPosIniCtg, 0047, StrZero( Val(aCab[2]), 3 ), oFont13) //Serie
	oDamdfe:Say(nLin-nPosIniCtg, 0075, StrZero( Val(aCab[1]), 9 ), oFont13) //Numero
...
/*/
//-----------------------------------------------------------------------
Static Function DamdfeVCP(oDamdfe, nLin, aCab )
	...
	//CUSTOMIZADO MANFIL
	//--- Relaccao de Notas
	oDamdfe:Say(nLin-nPosIniCtg, 0005, "Relação de Notas Emitidas", oFont14N)
	nLin += 12
	oDamdfe:Say(nLin-nPosIniCtg, 0005, "Série", oFont11N)
	oDamdfe:Say(nLin-nPosIniCtg, 0055, "Núm. Nota", oFont11N)
	oDamdfe:Say(nLin-nPosIniCtg, 0125, "Chave de Acesso", oFont11N)
	nLin += 5
	oDamdfe:FillRect({nLin-nPosIniCtg, 0005, (nLin + 3) - nPosIniCtg, 300}, oBrush1)

	//-- Dados Relaccao de Notas
	cSerieMDF	:= StrZero( Val(aCab[2]), tamsx3('F2_SERIE')[1])
	cNumMDF		:= StrZero( Val(aCab[1]), tamsx3('F2_DOC')[1]) 
	PNota(cSerieMDF, cNumMDF)
	dbSelectArea('TB1')
	nLinVeic   := nLin + 10
	while !TB1->(eof())

		oDamdfe:Say(nLinVeic-nPosIniCtg, 0005, TB1->F2_SERIE, oFont10N)
		oDamdfe:Say(nLinVeic-nPosIniCtg, 0055, TB1->F2_DOC, oFont10N)
		oDamdfe:Say(nLinVeic-nPosIniCtg, 0125, TB1->F2_CHVNFE, oFont10N)
		TB1->(dbSkip())
		nLinVeic += 10
	enddo
	TB1->(dbCloseArea())
	...
	//--- Vale Pedagio
	oDamdfe:Say(nLin-nPosIniCtg, 0332, "Vale Pedágio", oFont14N)
	nLin += 12
	oDamdfe:Say(nLin-nPosIniCtg, 0332, "Responsável CNPJ", oFont11N)
	oDamdfe:Say(nLin-nPosIniCtg, 0405, "Fornecedor CNPJ", oFont11N)
	oDamdfe:Say(nLin-nPosIniCtg, 0515, "Nro Comprovante", oFont11N)
	nLin += 5
	oDamdfe:FillRect({nLin-nPosIniCtg, 0332, (nLin + 3) - nPosIniCtg, 800}, oBrush1)
...
static function PNota(cSerMDF, cNumMDF)
	...
	beginsql alias 'TB1'
		select SF2.F2_SERIE, SF2.F2_DOC, SF2.F2_CHVNFE
		from
			%table:SF2%	SF2
		where
				SF2.%notdel%
			and SF2.F2_FILIAL	= %xFilial:SF2%
			and SF2.F2_SERMDF 	= %exp:cSerMDF%
			and SF2.F2_NUMMDF	= %exp:cNumMDF%
	endsql
