	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= ImpIbsCbsIs( aNota, cClieFor, cLoja )
				cString += '</rps>' 
			EndIf
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf,aRetSF3)	
	...
	for nX := 1 to len( aProd )
		...
		cString	+= RTPisCofins( aPisXml[3][nX], aCofinsXml[3][nX], cTribConfig )
		cString	+= "</servico>"
...
static function valores( aISSQN, aRetido, aTotal, aDest, cCodMun,aLeiTrp,lRecIrrf, aProd )
	...
	cString	+= RTPisCofins( aPisXml[1], aCofinsXml[1], cTribConfig )
	cString	+= "</valores>"
...
/*/{Protheus.doc}  Function ImpIbsCbsIs
	...
	@return void
/*///-----------------------------------------------------------------------
Static Function ImpIbsCbsIs( aNota, cClieFor, cLoja )
    Local cXmlIBSCBS    	:= ""
	Local cXmlIs    		:= ""
	Local cXml	  			:= ""
	Local cXmlTotal			:= ""
	Private cDocumentItemId := ""
	...
	cDocumentItemId := GetAdvFVal('SD2','D2_IDTRIB', SF2->(F2_FILIAL + F2_DOC + F2_SERIE + F2_CLIENTE + F2_LOJA), 3  )
	DbSelectArea("SD2")
	DbSetOrder(3)
	If DbSeek(xFilial("SD2")+aNota[2]+aNota[1]+cClieFor+cLoja)
		if !Empty(cDocumentItemId) .And. findClass('totvs.protheus.backoffice.tss.engine.xml.taxinformation')
			if oNfTciIntg <> Nil
				...
				cXmlTotal 	:= oXmlRefTri:getXmlTotalIbsCbsIs(AllTrim(SD2->D2_IDTRIB))
			EndIf

			if !Empty(cXmlIs)
				cXmlIs := "<impostois>" + cXmlIs + "</impostois>"
			EndIf
			
			if !Empty(cXmlIBSCBS)
				cXmlIBSCBS := "<impostoibscbs>" + cXmlIBSCBS + "</impostoibscbs>"
			EndIf
...
/*/{Protheus.doc}  Function RTPisCofins
	...
	@return string
/*///-----------------------------------------------------------------------
Static Function RTPisCofins( nAliqPisXml, nAliqCofinsXml, cIdTribPiSCof )
	Local cXml	  				:= ""
	Local cCSTPisCofins			:= ""
	...
	if nAliqPisXml > 0 .and. nAliqCofinsXml > 0 .and. !empty(cCSTPisCofins)
		cXml +=	'<piscofins>'
		...
		if nRetPis > 0 .or. nRetCof > 0 // Verifica se possui titulo de retenção de PIS ou COFINS na SE1
            cXml += '<tpRetPisCofins>1</tpRetPisCofins>'
        else
            cXml += '<tpRetPisCofins>2</tpRetPisCofins>'
            endif
		cXml += '</piscofins>'
	endif
...
Static Function GetCstPisCofinsRt( cIdTribPiSCof )
	...
	if !empty(cIdTribPiSCof)
		...
		if ( lConfTrib .And. ((oNfTciIntg:GetTax( cIdTribPiSCof, "PISRET") <> NIL ) .or. (oNfTciIntg:GetTax( cIdTribPiSCof, "COFRET") <> NIL ))) // Verifica impostos vieram do configurador de tributos
			if !empty(oNfTciIntg:GetEscrituracaoRuleTax( cIdTribPiSCof, "PISRET")['cst']) .or. !empty(oNfTciIntg:GetEscrituracaoRuleTax( cIdTribPiSCof, "COFINS")['cst'])
				if !empty(oNfTciIntg:GetEscrituracaoRuleTax( cIdTribPiSCof, "PISRET")['cst']) // Estrutura do governo possui tag unica para pis e cofins
