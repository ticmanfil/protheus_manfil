...
===============================================================================================================================
Descrição---------: Este programa serve para imprimir o DANFE pelo pedido de venda
===============================================================================================================================
...
Parametros--------: cIdEnt	= '000001'
					...
					cVal2	= .F.
===============================================================================================================================
Retorno-----------: impressao do DANFE em PDF
===============================================================================================================================
Chamado(SPS)------: 
===============================================================================================================================
Setor-------------: Faturamento
...
User Function RL05002(cIdEnt, cVal1, cVal2, lIsLoja, pDanfe, pBoleta, pViewPDF, pFunc	)
...
Private cArquivo  		:= "DANFE_"+cIdEnt+Dtos(MSDate())+StrTran(Time(),":","")
//Private cArquivo  		:= U_UChvNFE()
Private cDirUsr			:= alltrim(GetTempPath())	//'C:\temp'
...
if u_TemCHV(SF2->F2_DOC,SF2->F2_SERIE)
	u_msgErro('Nota ainda não autorizado!!!!','[RL05002] - Impressao DANFE')
	return .F.
endif 


oDanfe := FWMSPrinter():New(cArquivo, IMP_PDF, lAdjustToLegacy, , lDisabeSetup, .F., ,,,,,lViewPDF)
oDanfe:cPathPDF := "C:\TEMP\"
oDanfe:SetResolution(78) //Tamanho estipulado para a Danfe
oDanfe:SetPortrait()
oDanfe:SetPaperSize(DMPAPER_A4)
//oDanfe:SetMargin(60,60,60,60)
oDanfe:SetMargin(60,20,60,100)
oDanfe:lServer := .T.
...
//If lIsLoja
	if pFunc == "P"
		...
		MV_PAR02 := SC5->C5_NOTA
		MV_PAR03 := SC5->C5_SERIE
	else
		MV_PAR01 := SF2->F2_DOC 
		MV_PAR02 := SF2->F2_DOC
...
//impressao da observacao do pedido de venda (SC5)
/*If SC5->(FieldPos("C5_XOBS"))<>0
	cMemo	:= SC5->C5_XOBS
	For nXi := 1 To MLCount(cMemo,70)
		 If ! Empty(MLCount(cMemo,70))
			  If ! Empty(MemoLine(cMemo,70,nXi))
				cAux := "" 
				cAux += OemToAnsi(MemoLine(cMemo,70,nXi))
				While !Empty(cAux)
					aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
					cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
				EndDo
...
/*Static Function GetIdEnt(lConsulta)
	...
	//Â³Obtem o codigo da entidade                                              Â³
